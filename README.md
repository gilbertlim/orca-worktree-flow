...
모니터링은 표를 계속 긁는 일이 아니다. 우산이 낸 워크트리가 전부 커밋이 서고, 미커밋이 없고, 판정이 최신이고, blocking이 0이면 `status.sh`가 마지막에 그 줄을 찍는다.
...
우산 워크트리 자신은 이 셈에서 빠진다. 오케스트레이터가 앉아 있는 자리라 늘 더럽고, 그것까지 세면 "다 닫혔다"가 영영 안 뜬다.

셈에서만 빼고 제 줄은 따로 갖는다. 리뷰를 안 거치는 자리라 판정 파일은 안 보고, 미커밋이 없고 기준 브랜치보다 앞서 있으면 `▶ 이 우산 워크트리에도 main 앞에 커밋 3개가 서 있다`가 뜬다. 이 줄이 없으면 우산이 제 손으로 짠 것은 아무도 안 묻는다.
