...
워크트리는 같은 레포를 둘 이상이 동시에 수정할 때 만든다. 서로 다른 레포에서 병렬로 작업한다면 각 메인 체크아웃을 사용해도 된다. 디렉터리가 겹치지 않는데 워크트리를 추가하면 브랜치 관리, 머지와 push 확인만 늘어난다.

여러 레포가 함께 커밋돼야 하는 계약 변경에는 이 분리 방식을 사용하지 않는다. 브랜치가 나뉘면 머지가 끝날 때까지 계약이 깨진 중간 상태가 남을 수 있다.
